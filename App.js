...
export default function App() {
  ...
  const _handlePressButtonAsyncFB = async () => {
    let result = await WebBrowser.openBrowserAsync('https://www.facebook.com/AkbarHidayat1001/');
    <ScrollView>
      setResult(result);
    </ScrollView>
  };
  const _handlePressButtonAsyncIG = async () => {
    let result = await WebBrowser.openBrowserAsync('https://www.instagram.com/akbarrhdytt/');
    <ScrollView>
      setResult(result);
    </ScrollView>
  };
  const _handlePressButtonAsyncWA = async () => {
    let result = await WebBrowser.openBrowserAsync('https://wa.me/+6283187821826');
    <ScrollView>
      setResult(result);
    </ScrollView>
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.textProfil}>Akbar Hidayat</Text>
      <Text style={styles.textProfil}>119140004</Text>
      <View style={{height: '20%', justifyContent: 'center', marginTop: '10%'}}>
        <Image source={require("./assets/Akbar.jpg")} resizeMode= "contain" style={{height: '100%', width: '100%'}}/>
      </View>
